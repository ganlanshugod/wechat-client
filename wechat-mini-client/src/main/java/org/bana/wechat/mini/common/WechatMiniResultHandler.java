 ...
 * @author Liu Wenjie
 */
public class WechatMiniResultHandler {
	
	private static final Logger LOG = LoggerFactory.getLogger(WechatMiniResultHandler.class);
	...
	public static <T> T handleResult(JSONObject result,Class<T> cls){
		if(isSuccess(result)){
			...
			throw new WeChatMiniException(errcode, errmsg);
		}
	}
